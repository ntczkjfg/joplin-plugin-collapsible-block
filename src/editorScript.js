...
function addClassToLines(view, builder, lines, doEditorColors) {
    if (lines.length === 0) {
        return;
    }
...
function findCollapsibles(view, startToken, endToken, doEditorColors) {
    ...
    addClassToLines(view, builder, lines, doEditorColors);
...
export default (_context) => {
    return {
        ...
        plugin: async (codeMirrorWrapper) => {
            const settings = await _context.postMessage({ name: 'getSettings', data: {} });
            ...
            const collapsibleEditorColorsPlugin = ViewPlugin.fromClass(
                class {
                    constructor(view) {
                        ...
                        this.doEditorColors = doEditorColors;
                        this.decorations = findCollapsibles(view, this.startToken, this.endToken, this.doEditorColors);
                    }
                    update(update) {
                        if (update.docChanged || update.viewportChanged) {
                            this.decorations = findCollapsibles(update.view, this.startToken, this.endToken, this.doEditorColors);
                        }
                    }
                },
                {
